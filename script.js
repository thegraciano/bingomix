...
const GRUPOS = [
  ...
  ["\u{1F970}","\u{1F618}","\u{1F60B}","\u{1F61B}","\u{1F61C}","\u{1F92A}","\u{1F928}","\u{1F9D0}","\u{1F913}","\u{1F60E}","\u{1F929}","\u{1F973}","\u{1F63A}","\u{1F638}","\u{1F63B}"], // I
...
console.assert(new Set(TODOS).size === 75, "Existe emoji repetido na lista");

const LETRAS = ["B","I","N","G","O"];

/* ---------- 2) ESTADO E PERSISTENCIA ---------- */
const CHAVE = "bingoEmojis.v1";
...
function atualizarInfoCartela(){
  ...
  $("cartelaInfo").textContent = "Marcados: " + marcados + " de 25 \u00b7 toque nos emojis sorteados.";
}
...
function alternarMarca(i, botao){
  ...
  salvar();
  atualizarInfoCartela();
  if(cel.marcado) verificarBingo();
}

/* Detecta linha, coluna, diagonal ou cartela completa */
function linhasVencedoras(){
  const m = estado.cartela.map(c => c.marcado);
  const combos = [];
  for(let l = 0; l < 5; l++) combos.push({ tipo: "linha",   idx: [0,1,2,3,4].map(c => l*5+c) });
  for(let c = 0; c < 5; c++) combos.push({ tipo: "coluna",  idx: [0,1,2,3,4].map(l => l*5+c) });
  combos.push({ tipo: "diagonal", idx: [0,6,12,18,24] });
  combos.push({ tipo: "diagonal", idx: [4,8,12,16,20] });

  const vencedores = combos.filter(k => k.idx.every(i => m[i]));
  const completa = m.every(Boolean);
  return { vencedores, completa };
}

let ultimoBingo = ""; // evita repetir a tela de vitoria para o mesmo bingo
function verificarBingo(){
  const { vencedores, completa } = linhasVencedoras();
  ...
  vencedores.forEach(v => v.idx.forEach(i => celulas[i] && celulas[i].classList.add("is-win")));

  const assinatura = (completa ? "full" : "") + vencedores.map(v => v.idx.join("")).join("|");
  if(assinatura === ultimoBingo) return;
  ultimoBingo = assinatura;

  const tipo = completa ? "a cartela completa" :
    (vencedores.length > 1 ? vencedores.length + " sequencias" : "uma " + vencedores[0].tipo);
  $("winSub").textContent = "Voce completou " + tipo + "!";
...
async function sortear(){
  ...
  emojiEl.classList.add("is-counting");
  ...
  // revela o emoji
  emojiEl.classList.remove("is-counting");
